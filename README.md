...
1. 유저 카카오 이름, 유저이미지 id 반환 API
2. 마이페이지 하단
   - 코스 목록 리스트(여행id, 여행이름, 대표사진, 여행날짜, 일기작성(enum) 반환) API - 안간 거 젤 먼저, 그 이후는 최신순
   - 여행 id값 받아서 일정 삭제 API
   - 일기 리스트(일기id, 대표사진, 여행이름, 날짜) - 최신순
3. 여행id 받아서 상세정보 반환api(여행이름, 여행날짜, 좌표, 코스 목록(장소명, 좌표, 사진 등))

**[클라]**

1. 랜덤이미지 프로필설정 → id 받아야함!!!!!
2. 유저 조회 api → 이름, 태그, 나의 일정리스트, 일기리스트 받아오면 탭 나눠서 보여주기
   [일정리스트]

- 일정리스트(여행 id, 여행이름, 대표사진, 여행날짜, 같이간 사람 이름들)를 일기작성enum에 따라 표시를 달리하기
- 일정 삭제 api 요청 → 일정 id 주기
- 가기전이면 일정상세 api 요청 (여행id주기) → 받아온 여행들고 상세페이지 뿌리기
- 일기만들기 버튼 (여행id 주기) → 반환값+여행명, 여행날짜, 여행지 들고 일기작성페이지 고고

[일기]

- 일기id, 대표사진, 여행이름, 여행날짜
- 누르면 일기 디테일 api요청 (일기id주기) → 반환값 + 여행명, 여행날짜, 여행지 들고가야함
