...
def getLikeCbfList(request, user_id):
    ...
    # 유사한 장소들의 DESTINATION_ID만을 포함한 리스트로 변환
    similar_destinations_ids = [destination.DESTINATION_ID for destination in similar_destinations]

    return Response(similar_destinations_ids, status=status.HTTP_200_OK)
...
def getLikeCbfDetail(request, user_id):
    ...
    # 추천 결과를 직렬화
    # serializer = DestinationSerializer(similar_destinations[:30], many=True)
