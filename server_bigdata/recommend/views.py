...
from .algo.main_list import genderAgeRecommend, basicCbfRecommend, basicCfRecommend
from .algo.travel_list import travelRecommend
from .algo.city_list import cityRecommend
...
def getMainList(request, user_id):
    ...
    if user_likes_count <= 20:
        ...
        gender_age_response = genderAgeRecommend(user_id,8)
        basic_response = basicCbfRecommend(user_id)
    else:
        # 기준점 초과일 때 user-based 추천(4) + CF(100)
        gender_age_response = genderAgeRecommend(user_id,4)
        basic_response = basicCfRecommend(user_id)
...
def getTravelList(request):
    all_destination_ids = travelRecommend(request.data)
